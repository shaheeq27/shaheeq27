...
GRID_COLS = 100
# GRID_ROWS is set so the rendered content block's aspect ratio matches
# the source photo's own aspect ratio, accounting for non-square
# character cells (CHAR_W=4.8, CHAR_H=8.0 below). The original value
# (53) did not do this - it produced a content block wider, relative to
# its height, than the source photo actually is, which meant every
# portrait generated by this script was already being subtly squashed
# horizontally during the resize() resampling step, independent of any
# canvas/padding choices made elsewhere. 93 is correct specifically for
# a 792x1228 source photo (ratio 0.645); if the source photo's own
# aspect ratio changes, recompute: GRID_ROWS = GRID_COLS * CHAR_W /
# (CHAR_H * (photo_width / photo_height)).
GRID_ROWS = 67
...
# Optional canvas override: when set, the canvas is built at this height
# (content-block height + top/bottom margins otherwise) and the ASCII
# content block is centered vertically inside it, undistorted. This
# exists to let the portrait's rendered height match another SVG
# (info-card.svg) when both are displayed at different README <img
# width=...> values with different native aspect ratios - purely a
# canvas/padding change, the character grid and its own proportions
# are untouched either way.
CANVAS_HEIGHT_OVERRIDE = None  # e.g. 754 - set via --canvas-height flag
...
def build_svg(grid: list[list[str]], canvas_height_override: float | None = None) -> str:
    width = GRID_COLS * CHAR_W + 20
    content_height = GRID_ROWS * CHAR_H + 20  # original content-hugging height

    if canvas_height_override is not None and canvas_height_override > content_height:
        height = canvas_height_override
        # Center the (unchanged, undistorted) content block vertically -
        # this is the only thing that differs from the default: where
        # the first row's baseline starts, not the grid, font size, or
        # character spacing.
        top_offset = (height - content_height) / 2
    else:
...
def main():
    ...
    src = args[0] if len(args) > 0 else "source-prepped.png"
    out = args[1] if len(args) > 1 else "avi-ascii.svg"

    canvas_height = None
    for a in sys.argv[1:]:
        if a.startswith("--canvas-height="):
            canvas_height = float(a.split("=", 1)[1])
    ...
    svg = build_svg(grid, canvas_height_override=canvas_height)

    Path(out).write_text(svg, encoding="utf-8")
    print(f"[make_ascii_svg] wrote {out}" + (f" (canvas height forced to {canvas_height}px)" if canvas_height else ""))
